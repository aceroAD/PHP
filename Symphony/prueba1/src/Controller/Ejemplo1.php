...
class Ejemplo1 extends AbstractController{
	...
	*/
	public function factorioal($num) {
		$resultado = 1;
		if($num==0)
			$resultado=1;
		else for($int = $resultado; $int<=$num; $int++) {
			$resultado = $resultado * $int;
		}
		return new Response('<html><body>'.$resultado.'</body></html>');
	}
	
	/**
	 * @Route("/resto/{num}/{num2?2}", name="resto")
	 */
	public function resto($num,$num2) {
	    $resultado = $num % $num2;
	    return new Response('<html><body>'.$resultado.'</body></html>');
	}
