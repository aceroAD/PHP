...
class EjemploBaseDatos extends AbstractController{
     ...
     */
	 ...
	 /** Actividad 8.4
     * @Route("/equipo/{num}", name="equipo")
     */
    public function equipo($num){		
		$error = FALSE;
		$entityManager = $this->getDoctrine()->getManager();		
		$eq = $entityManager->find(EquipoBidireccional::class, $num);
		if(is_null($eq)){
			$error = TRUE;
		}
		return $this->render('equipo.html.twig', array ('error' => $error, 'equipo' => $eq));
	}
